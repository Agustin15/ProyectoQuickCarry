<?php


require '../../../../../model/conexion.php';

class datosUsuario
{


    private $conexion;

    public function __construct()
    {
        $this->conexion = new conexion();
    }



    public function traerDatos()
    {


        $registros =$this->conexion->bd()->prepare("select * from chofercamion");
        $registros->execute();

        $registro = $registros->get_result();


        return $registro;
    }



    public function getCamiones()
    {


        $registros =$this->conexion->bd()->prepare("select * from camiones");
        $registros->execute();

        $registro = $registros->get_result();
        

        return $registro;
    }

    public function eliminarUsuario($id)
    {

        $consulta = $this->conexion->bd()->prepare("delete from chofercamion where id=?");
        $consulta->bind_param('i', $id);
        $consulta->execute();


        return $consulta;
    }

    public function modificarUsuario(
        $nombre,
        $apellido,
        $usuario,
        $numCamionero,
        $matriculaCamion,
        $cedula,
        $correo,
        $id
    ) {

        $sentencia = $this->conexion->bd()->prepare("update chofercamion 
        set nombre=?,apellido=?,usuario=?,numChoferCamion=?,matriculaCamion=?,cedula=?,correo=? where id=?");

        $sentencia->bind_param(
            'sssisssi',
            $nombre,
            $apellido,
            $usuario,
            $numCamionero,
            $matriculaCamion,
            $cedula,
            $correo,
            $id
        );
        $sentencia->execute();


        return $sentencia;
    }




    public function comprobarCedulaActualizado($cedula, $id)
    {

        $repetido = false;

        $seleccionIdCrecom = $this->conexion->bd()->prepare("select * from chofercamion where id=?");
        $seleccionIdCrecom->bind_param("i", $id);
        $seleccionIdCrecom->execute();
        $registros = $seleccionIdCrecom->get_result();
        $regIdCrecom = $registros->fetch_array();

        if ($regIdCrecom['cedula'] != $cedula) {


            $seleccionFuncioCrecom =$this->conexion->bd()->prepare("select * from chofercamion  where cedula=?");
            $seleccionFuncioCrecom->bind_param("i", $cedula);
            $seleccionFuncioCrecom->execute();
            $registros = $seleccionFuncioCrecom->get_result();
            $regFuncioCrecom = $registros->fetch_array();


            if ($regFuncioCrecom > 0) {

                $repetido = true;
            } else {

                $repetido = false;
            }
        }

        return $repetido;
    }




    public function comprobarCorreoActualizado($correo, $id)
    {

        $repetido = false;

        $seleccionIdCrecom = $this->conexion->bd()->prepare("select * from chofercamion where id=?");
        $seleccionIdCrecom->bind_param("i", $id);
        $seleccionIdCrecom->execute();
        $registros = $seleccionIdCrecom->get_result();
        $regIdCrecom = $registros->fetch_array();

        if ($regIdCrecom['correo'] != $correo) {



            $seleccionUsuarioCrecom =$this->conexion->bd()->prepare("select * from chofercamion where correo=?");
            $seleccionUsuarioCrecom->bind_param("s", $correo);
            $seleccionUsuarioCrecom->execute();
            $registros = $seleccionUsuarioCrecom->get_result();
            $regUsuarioCrecom = $registros->fetch_array();


            if ($regUsuarioCrecom > 0) {

                $repetido = true;
            } else {

                $repetido = false;
            }
        }

        return $repetido;
    }




    public function comprobarUsuarioActualizado($usuario, $id)
    {

        $repetido = false;

        $seleccionIdQuick = $this->conexion->bd()->prepare("select * from chofercamion where id=?");
        $seleccionIdQuick->bind_param("i", $id);
        $seleccionIdQuick->execute();
        $registros = $seleccionIdQuick->get_result();
        $regIdQuick = $registros->fetch_array();


        if ($regIdQuick['usuario'] != $usuario) {


            $seleccionUsuarioQuick =$this->conexion->bd()->prepare("select * from chofercamion where usuario=?");
            $seleccionUsuarioQuick->bind_param("s", $usuario);
            $seleccionUsuarioQuick->execute();
            $registros = $seleccionUsuarioQuick->get_result();
            $regUsuarioQuick = $registros->fetch_array();



            if ($regUsuarioQuick > 0) {

                $repetido = true;
            } else {

                $repetido = false;
            }
        }

        return $repetido;
    }


    public function comprobarNumCamioneroActualizado($numCamionero, $id)
    {

        $repetido = false;

        $seleccionIdQuick = $this->conexion->bd()->prepare("select * from chofercamion where id=?");
        $seleccionIdQuick->bind_param("i", $id);
        $seleccionIdQuick->execute();
        $registros = $seleccionIdQuick->get_result();
        $regIdQuick = $registros->fetch_array();

        if ($regIdQuick['numChoferCamion'] != $numCamionero) {


            $seleccionChofer = $this->conexion->bd()->prepare("select * from chofercamion where numChoferCamion=?");
            $seleccionChofer->bind_param("i", $numCamionero);
            $seleccionChofer->execute();
            $registros = $seleccionChofer->get_result();
            $regChofer = $registros->fetch_array();



            if ($regChofer > 0) {

                $repetido = true;
            } else {

                $repetido = false;
            }
        }

        return $repetido;
    }



    public function comprobarMatriculaActualizado($matricula, $id)
    {

        $repetido = false;

        $repetido = false;

        $seleccionIdQuick = $this->conexion->bd()->prepare("select * from chofercamion where id=?");
        $seleccionIdQuick->bind_param("i", $id);
        $seleccionIdQuick->execute();
        $registros = $seleccionIdQuick->get_result();
        $regIdQuick = $registros->fetch_array();

        if ($regIdQuick['matriculaCamion'] != $matricula) {


            $repetido = false;

            $seleccionMatriculaQuick = $this->conexion->bd()->prepare("select * from chofercamion where matriculaCamion=?");
            $seleccionMatriculaQuick->bind_param("s", $matricula);
            $seleccionMatriculaQuick->execute();
            $registros = $seleccionMatriculaQuick->get_result();
            $regMatriculaQuick = $registros->fetch_array();


            if ($regMatriculaQuick > 0) {

                $repetido = true;
            } else {

                $repetido = false;
            }
        }

        return $repetido;
    }

    public function agregarUsuario(
        $nombre,
        $apellido,
        $usuario,
        $numCamionero,
        $matriculaCamion,
        $cedula,
        $correo,
        $contrasenia
    ) {

        $sentencia = $this->conexion->bd()->prepare("insert into chofercamion
        (nombre,apellido,usuario,numChoferCamion,matriculaCamion,cedula,correo,contrasenia) 
         values(?,?,?,?,?,?,?,?)");

        $sentencia->bind_param(
            'sssissss',
            $nombre,
            $apellido,
            $usuario,
            $numCamionero,
            $matriculaCamion,
            $cedula,
            $correo,
            $contrasenia
        );

        $sentencia->execute();


        return $sentencia;
    }



    public function masInfoUsuario($id)
    {

        $registros = $this->conexion->bd()->prepare("select * from chofercamion where id=?");
        $registros->bind_param("i", $id);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }


    public function comprobarUsuarioRepetidoChofer($usuario)
    {

        $registros = $this->conexion->bd()->prepare("select * from chofercamion where usuario=?");
        $registros->bind_param("s", $usuario);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }


    public function comprobarUsuarioRepetidoMatricula($matricula)
    {

        $registros = $this->conexion->bd()->prepare("select * from chofercamion where matriculaCamion=?");
        $registros->bind_param("s", $matricula);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }



    public function comprobarCedulaRepetidoChofer($cedula)
    {

        $registros = $this->conexion->bd()->prepare("select * from chofercamion where cedula=?");
        $registros->bind_param("s", $cedula);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }


    public function comprobarCorreoRepetidoChofer($correo)
    {

        $registros = $this->conexion->bd()->prepare("select * from chofercamion where correo=?");
        $registros->bind_param("s", $correo);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }

    public function comprobarUsuarioRepetidoCrecom($usuario)
    {

        $registros = $this->conexion->bd()->prepare("select * from funcionarioscrecom where usuario=?");
        $registros->bind_param("s", $usuario);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }


    public function comprobarNumCamioneroRepetido($numCamionero)
    {

        $registros = $this->conexion->bd()->prepare("select * from chofercamion where numChoferCamion=?");
        $registros->bind_param("i", $numCamionero);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }






    public function comprobarUsuarioRepetidoAdmin($usuario)
    {

        $registros = $this->conexion->bd()->prepare("select * from admin where usuario=?");
        $registros->bind_param("s", $usuario);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }


    public function comprobarUsuarioRepetidoFuncioQuick($usuario)
    {

        $registros = $this->conexion->bd()->prepare("select * from funcionariosquickcarry where usuario=?");
        $registros->bind_param("s", $usuario);
        $registros->execute();

        $registro = $registros->get_result();

        $reg = $registro->fetch_array();
        return $reg;
    }

    

    public function comprobarUsuarioRepetidoClienteCrecom($usuario)
    {

        $seleccionUsuario =$this->conexion->bd()->prepare("select * from clientecrecom where usuario=?");
        $seleccionUsuario->bind_param("s", $usuario);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }


    public function comprobarCedulaRepetidoFuncioQuick($cedula)
    {

        $seleccionUsuario = $this->conexion->bd()->prepare("select * from funcionariosquickcarry where cedula=?");
        $seleccionUsuario->bind_param("s", $cedula);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }



    public function comprobarCedulaRepetidoCrecom($cedula)
    {

        $seleccionUsuario = $this->conexion->bd()->prepare("select * from funcionarioscrecom where cedula=?");
        $seleccionUsuario->bind_param("s", $cedula);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }





    public function comprobarCorreoRepetidoCrecom($correo)
    {

        $seleccionUsuario = $this->conexion->bd()->prepare("select * from funcionarioscrecom where correo=?");
        $seleccionUsuario->bind_param("s", $correo);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }


    public function comprobarCorreoRepetidoAdmin($correo)
    {

        $seleccionUsuario =$this->conexion->bd()->prepare("select * from admin where correo=?");
        $seleccionUsuario->bind_param("s", $correo);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }

    public function comprobarCorreoRepetidoFuncioQuick($correo)
    {

        $seleccionUsuario = $this->conexion->bd()->prepare("select * from funcionariosquickcarry where correo=?");
        $seleccionUsuario->bind_param("s", $correo);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }


    

    public function comprobarCorreoRepetidoClienteCrecom($correo)
    {

        $seleccionUsuario = $this->conexion->bd()->prepare("select * from clientecrecom where correo=?");
        $seleccionUsuario->bind_param("s", $correo);
        $seleccionUsuario->execute();
        $registros = $seleccionUsuario->get_result();
        $regUsuario = $registros->fetch_array();


        return $regUsuario;
    }
}
